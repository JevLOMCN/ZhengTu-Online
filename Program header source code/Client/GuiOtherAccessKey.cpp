...
int g_nOtherAccessBtnLeft = 5;
int g_nOtherAccessBtnTop = 5;	
int g_OtherAccessBtnSpace = 39;

DWORD dwStateO[] = { GUI_STATE_NORMAL,GUI_STATE_PRESSED,GUI_STATE_DISABLED };
/**
 * \brief 构造函数
 * 
 * 
 ...
 * \return 
 */
CGuiOtherAccessKey::CGuiOtherAccessKey(void)
{
	FUNCTION_BEGIN;
	
	for( int i=0;i<OTHER_ACCESS_KEY_NUM;i++ )
 ...
 * \return 
 */
void CGuiOtherAccessKey::OnClose(void)
{
	FUNCTION_BEGIN;
	...
	GetGameGuiManager()->m_guiOtherAccessKey = NULL;
 ...
 * \return 
 */
void CGuiOtherAccessKey::OnCreate()
{
	FUNCTION_BEGIN;
	...
	for( int i=0;i<OTHER_ACCESS_KEY_NUM;i++ )
	{
		...
		{
			/* if ( i != 0 )
				m_pBtnAccessKey[i]->SetLocation( g_nOtherAccessBtnLeft+(i-1)*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop );
			else
				m_pBtnAccessKey[i]->SetLocation( g_nOtherAccessBtnLeft+(OTHER_ACCESS_KEY_NUM-1)*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop ); */

			m_pBtnAccessKey[i]->SetLocation( g_nOtherAccessBtnLeft+(i)*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop );
 ...
 * \return 
 */
void CGuiOtherAccessKey::OnResetScreenSize()
{
	if ( m_bPretendedHide )
	{
		m_x = -10000;
...
* \return 
*/
void CGuiOtherAccessKey::__SetEmotionAccessBtn(stResourceLocation & rl,int btnid)
{
	FUNCTION_BEGIN;

	/////////////////////////////////////////////////////////////////////////////////
	// Safe checking
	if( btnid < 0  || btnid >= OTHER_ACCESS_KEY_NUM)
		return;
	...
	m_pBtnAccessKey[btnid]->SetLocation( g_nOtherAccessBtnLeft+btnid*g_OtherAccessBtnSpace+ptOffset.x, g_nOtherAccessBtnTop+ptOffset.y );
	/////////////////////////////////////////////////////////////////////////////////
 ...
 * \return 
 */
inline void ParseOtherAccessBtnClickEvent( int id )
{
	FUNCTION_BEGIN;
	...
	if ( GetGameGuiManager()->m_guiOtherAccessKey )
	{
		if ( (GetGameCursor()->IsDrag() && ((GetGameCursor()->GetCursorImageType() == eCursorSetAccess) || ( GetGameCursor()->GetCursorImageType() == eCursorDictate) ))  || GetGuiManager()->GetFloatItem() )
			GetGameGuiManager()->m_guiOtherAccessKey->SetAccessKey( id );
		else
		{
			...
			{
				if ( (id != -1)
					&& !(GetGameAccelKeyManager()->m_curEditAccessKey.placeAccessKe == PLACE_ACCESSKEY_OTHER && (GetGameAccelKeyManager()->m_curEditAccessKey.keyNo == id)) )		//Cur pos has access key btn
				{
					GetGameGuiManager()->m_guiOtherAccessKey->EditAccessKeyBtn( id );
				}
						...
						&& ( (GetGameGuiManager()->m_guiOtherAccessKey->m_pBtnAccessKey[id]->GetDragOffset().x == 0)
							&& (GetGameGuiManager()->m_guiOtherAccessKey->m_pBtnAccessKey[id]->GetDragOffset().y == 0) ) )
				{
					RestoreMouseIcon();
				}
			}
			else
			{
				...
				DWORD dwKeyID;
				BYTE curGroup = OTHER_ACCESS_KEY_GROUP;
				BYTE key[4];
				key[0] = VK_CONTROL;
				key[1] = VK_F1 + id;
				key[2] = 0;
					...
					case GameAccelKeyType_Skill:
						{
							...
							if ( pSelectSkill )
							{
								...
								{
									if ( GetGameGuiManager()->m_guiOtherAccessKey )
										GetGameGuiManager()->m_guiOtherAccessKey->OnFindAccessSkill();
								}
								else
								{
									...
									else
									{
											...
											/*&& !IsSelectFirstSkill( pSelectSkill )*/ )
										{
											...
											{
												if ( GetGameGuiManager()->m_guiOtherAccessKey )
													GetGameGuiManager()->m_guiOtherAccessKey->OnAccessBtnSelectSkill( id, dwKeyID );
											}                                                                                      											
										}
										else
										{
					...
					case GameAccelKeyType_Face:
						{
							...
							int fNum = 0;
							if ( GetGameGuiManager()->m_guiOtherAccessKey->m_pBtnAccessKey[id]->GetBitmapInfo( rl, fNum ) )
							{	
								...
								GetGameAccelKeyManager()->m_curEditAccessKey.placeAccessKe = PLACE_ACCESSKEY_OTHER;
								GetGameAccelKeyManager()->m_curEditAccessKey.group = OTHER_ACCESS_KEY_GROUP;
								GetGameAccelKeyManager()->m_curEditAccessKey.keyNo = id;
								...
								CGuiButton* btn = GetGameGuiManager()->m_guiOtherAccessKey->m_pBtnAccessKey[id];
 ...
 * \return 
 */
void CGuiOtherAccessKey::OnInitScript()
{
	FUNCTION_BEGIN;
	...
	[
		def("ParseOtherAccessBtnClickEvent",&ParseOtherAccessBtnClickEvent)
	];
 ...
 * \return 
 */
HRESULT CGuiOtherAccessKey::RenderAccessKeyNotes( float fElapsedTime )
{
	FUNCTION_BEGIN;
	...
	for( int i=0;i<OTHER_ACCESS_KEY_NUM;i++ )
	{
		if ( m_pBtnAccessKey[i] )
		{
			...
			stPointI pt;
	        /*    	if ( i != 0 )
				 pt.x =  g_nOtherAccessBtnLeft+(i-1)*g_OtherAccessBtnSpace;
			else
				 pt.x =  g_nOtherAccessBtnLeft+(OTHER_ACCESS_KEY_NUM-1)*g_OtherAccessBtnSpace; */
			pt.x = g_nOtherAccessBtnLeft+(i)*g_OtherAccessBtnSpace-2;
			pt.y = g_nOtherAccessBtnTop ;
		...
		//	pt = m_pBtnAccessKey[i]->GetLocation();
			...
			char keyName[4];
			sprintf( keyName, "F%d", i+1 );
			GetDevice()->DrawString( keyName, pt );
			
			stPointI pt2;
			pt2.x = (i)*g_OtherAccessBtnSpace + m_x + 8;
			pt2.y = m_y + 5;
			char ctrlName[5];
			sprintf( ctrlName, "Ctrl");
			GetDevice()->DrawString( ctrlName, pt2 );
			...
			key[1] = VK_F1 + i;
			key[2] = 0;
			if ( GetGameAccelKeyManager()->GetAccessKeyIdAndType( key, OTHER_ACCESS_KEY_GROUP, type, id ) )
			{
							...
							|| (pItem->GetLocation().dwLocation == OBJECTCELLTYPE_STORE) ) )
					{
						...
						if ( GetScene()->GetMainCharacter() && (type == GameAccelKeyType_Item) && !pItem->IsAccessNeedThisID() )
						{
							...
							else
							{
								...
								GetGameAccelKeyManager()->DeleteOneAccelKey( key, OTHER_ACCESS_KEY_GROUP );
 ...
 * \return 
 */
void CGuiOtherAccessKey::UpdateKeyTooltips( const BYTE& btnID )
{
	FUNCTION_BEGIN;
	...
	if ( GetGameAccelKeyManager()->GetAccessKeyIdAndType( key, (BYTE)OTHER_ACCESS_KEY_GROUP, type, id ) )
	{
		switch( type )
		{
		case GameAccelKeyType_Skill:
...
void CGuiOtherAccessKey::OnAccessBtnSelectSkill( const int& btnID, const int& skillId )
{
...
void CGuiOtherAccessKey::EditAccessKeyBtn( const DWORD& id )
{
	if ( GetGameAccelKeyManager() && GetGameAccelKeyManager()->m_curEditAccessKey.IsValid() )
	{
		...
		if( GetGameAccelKeyManager()->GetCurAccelKeyID(keyType,dwKeyID, curGroup, key) )		//cur btn has an access key
		{
			...
			if ( GetGameAccelKeyManager()->m_curEditAccessKey.placeAccessKe == PLACE_ACCESSKEY_OTHER )
			{
				Engine_WriteLog("自己和自己交换\n");
				//Exchange access key
				//Change btn bitmap
					...
					&& m_pBtnAccessKey[id] )
				{
					...
					if ( keyType == GameAccelKeyType_Skill || keyType == GameAccelKeyType_TaoZhuang || keyType == GameAccelKeyType_AttPak)
					{
						...
						m_pBtnAccessKey[GetGameAccelKeyManager()->m_curEditAccessKey.keyNo]->SetBitmap( rl, 3, dwStateO );
					}
					...
					m_pBtnAccessKey[id]->SetLocation( g_nOtherAccessBtnLeft+id*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop );
						...
						|| GetGameAccelKeyManager()->m_curEditAccessKey.keyType == GameAccelKeyType_AttPak)
					{
						...
						m_pBtnAccessKey[id]->SetBitmap( rlOld, 3, dwStateO );
					}
				...
				//Add new two
				{
					...
					//add new cur
					{
						...
						key.szAccelKey[0] = VK_CONTROL;
						key.szAccelKey[1] = VK_F1 + id;
						key.szAccelKey[2] = 0;
						key.mGroup = (BYTE)OTHER_ACCESS_KEY_GROUP;

						GetGameAccelKeyManager()->SetKey(key);
						GetGameAccelKeyManager()->Save( OTHER_ACCESS_KEY_GROUP );
					}
				}
					...
					&& GetGameGuiManager()->m_guiMain->IsVisible() )
				{
						...
						&& m_pBtnAccessKey[id] )
					{
						...
						if ( keyType == GameAccelKeyType_Skill || keyType == GameAccelKeyType_TaoZhuang || keyType == GameAccelKeyType_AttPak )
						{
							...
							pMaindlg->m_pBtnAccessKey[GetGameAccelKeyManager()->m_curEditAccessKey.keyNo]->SetBitmap( rl, 3, dwStateO );
						}
						else if( keyType == GameAccelKeyType_Face)
						{
						...
						&& GetGameGuiManager()->m_guiFloatAcessKey->IsVisible() )
					{
						...
						&& m_pBtnAccessKey[id] )
						{
							...
							if ( keyType == GameAccelKeyType_Skill ||  keyType == GameAccelKeyType_TaoZhuang ||  keyType == GameAccelKeyType_AttPak)
							{
								...
								pFloatAccessKeydlg->m_pBtnAccessKey[GetGameAccelKeyManager()->m_curEditAccessKey.keyNo]->SetBitmap( rl, 3, dwStateO );
							}
							else if( keyType == GameAccelKeyType_Face)
							{
		...
		else		//cur btn has no access key
		{
			...
			if ( GetGameAccelKeyManager()->m_curEditAccessKey.placeAccessKe == PLACE_ACCESSKEY_OTHER )
			{
				...
				{
					m_pBtnAccessKey[id]->SetBitmapScale( skillIconScale );
					m_pBtnAccessKey[id]->SetBitmap( GetGameAccelKeyManager()->m_curEditAccessKey.rl, 3, dwStateO );
				}
...
bool CGuiOtherAccessKey::OnGuiEvent( UINT nEvent,UINT nID,CGuiControl* pControl )
{
	FUNCTION_BEGIN;
	...
	case EVENT_BUTTON_DRAG_MOVE:
		if ( (nID >= 100) && (nID < 100+OTHER_ACCESS_KEY_NUM) )
		{
			nID -= 100;
			if ( !GetGameCursor()->IsDrag()
	...
	case EVENT_BUTTON_DRAG_END:
		...
		//						if ( GetGameAccelKeyManager()->DeleteOneAccelKey( key, (BYTE)OTHER_ACCESS_KEY_GROUP ) )
		//						{
	...
	case EVENT_LBUTTON_DOWN:
		...
		//		enumGameAccelKeyType keyType;
		//		DWORD dwKeyID;
		//		BYTE curGroup = (BYTE)OTHER_ACCESS_KEY_GROUP;
		//		BYTE key[4];
		//		key[0] = '0' + nID - 100;
		//		key[1] = 0;
		//		if( GetGameAccelKeyManager()->GetCurAccelKeyID(keyType,dwKeyID, curGroup, key) )
 ...
 * \return 鼠标指向的快捷键按钮id
 */
int CGuiOtherAccessKey::GetMouseOverAccessKeyNo()
{
	FUNCTION_BEGIN;
 ...
 * \return 
 */
HRESULT CGuiOtherAccessKey::RenderAccessKeyBtnToolTips( float fElapsedTime )
{
	FUNCTION_BEGIN;

	if ( (m_mouseOverAccessKeyNo < 0) || (m_mouseOverAccessKeyNo >= OTHER_ACCESS_KEY_NUM ) )
		return S_OK;
	...
	BYTE key[4];
	key[0] = VK_CONTROL;
	key[1] = VK_F1 + m_mouseOverAccessKeyNo;
	key[2] = 0;
	if ( GetGameAccelKeyManager()->GetAccessKeyIdAndType( key, OTHER_ACCESS_KEY_GROUP, type, id ) )
	{
		//Get offset cause of different screen size
 ...
 * \return 
 */
HRESULT CGuiOtherAccessKey::RenderAccessKeyBtnDisableStatus( float fElapsedTime )
{
	FUNCTION_BEGIN;
...
void CGuiOtherAccessKey::ResetDlgLocation()
{
	//Set pos follow chat select dlg
	if ( GetGameGuiManager()->m_guiChatSelect && GetGameGuiManager()->m_guiChatSelect->IsVisible() )
	{
		//m_x = GetGameGuiManager()->m_guiChatSelect->GetLocation().x + 422;
		m_x = GetGameGuiManager()->m_guiChatSelect->GetLocation().x;
 ...
 * \return 
 */
HRESULT CGuiOtherAccessKey::OnRender( float fElapsedTime )
{
	FUNCTION_BEGIN;
...
bool CGuiOtherAccessKey::OnFindAccessSkill()
{
 ...
 * \return 
 */
bool CGuiOtherAccessKey::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	FUNCTION_BEGIN;
 ...
 * \return 
 */
void CGuiOtherAccessKey::DeleteAccessKey( BYTE key )
{
	FUNCTION_BEGIN;

	RemoveAccessKeyBtnInfo( key);
 ...
 * \return 
 */
void CGuiOtherAccessKey::RemoveAccessKeyBtnInfo( BYTE key )
{
	FUNCTION_BEGIN;

	if ( key<0 || key >= OTHER_ACCESS_KEY_NUM )
		return;
 ...
 * \return 
 */
void CGuiOtherAccessKey::DeleteMouseOverAccessKey()
{
	FUNCTION_BEGIN;
 ...
 * \return 
 */
void CGuiOtherAccessKey::RefreshKeyGroupState()
{
	FUNCTION_BEGIN;
			...
			case GameAccelKeyType_Skill:
				{
					if ( GetGameGuiManager()->m_guiSkill->GetSkillIconResourceFromID( id, rl ) )
					{
						...
						m_pBtnAccessKey[i]->SetBitmap( rl, 3, dwStateO );
					}
					else
					{
 ...
 */
bool CGuiOtherAccessKey::SetAccessKey( const int& keyID )
{
	FUNCTION_BEGIN;
	if ( (keyID >= OTHER_ACCESS_KEY_NUM) || (keyID < 0) )
	{
		RestoreMouseIcon();
		return false;
	}
	...
	/* if ( keyID != 0 )
		m_pBtnAccessKey[keyID]->SetLocation( g_nOtherAccessBtnLeft+(keyID-1)*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop );
	else
		m_pBtnAccessKey[keyID]->SetLocation( g_nOtherAccessBtnLeft+(OTHER_ACCESS_KEY_NUM-1)*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop ); */
	m_pBtnAccessKey[keyID]->SetLocation( g_nOtherAccessBtnLeft + keyID*g_OtherAccessBtnSpace, g_nOtherAccessBtnTop );
	stResourceLocation rl;
	switch( type )
	{
	case CGuiMain::ActiveAccessSkill:
		{
			if ( GetGameGuiManager()->m_guiSkill->GetActiveSkillIconResource( rl ) )
			{
				...
				m_pBtnAccessKey[keyID]->SetBitmap( rl, 3, dwStateO );
	...
	case CGuiMain::ActiveAccessEmotion:
		{
			...
			else  if( FROM_EMOTION == _from)
			{
				if( GetEmotionInfo()->GetActiveResourceLocation(rl) )
				{
					...
					key.szAccelKey[1] = VK_F1 + keyID;
					key.szAccelKey[2] = 0;
					key.mGroup = (BYTE)OTHER_ACCESS_KEY_GROUP;  // must be OTHER_ACCESS_KEY_GROUP ,but not EMOTION_ACCESS_KEY_GROUP
