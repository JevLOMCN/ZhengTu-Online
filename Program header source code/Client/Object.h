...
enum enumItemType
{
	...
	ItemType_SkillUp = 44,   //44 代表技能升级道具
	...
	ItemType_HzStone = 78,  //合成魂魄石
   	...
   	ItemType_FillXHP = 72, //soke 72代表自动补血道具
	...
	ItemType_HorseFashion=140,	    //140代表马匹圣石
...
#define BOW_ARROW_ITEM_ID 660
#define CROSSBOW_ARROW_ITEM_ID 661
#define CANNOT_SELL_ARROW_ID 800
#define LARGE_AMMOUNT_ARROW_ID	801

#define ARROW_TYPE_NUM	4
static const DWORD arrow_type[ARROW_TYPE_NUM] = { BOW_ARROW_ITEM_ID, CROSSBOW_ARROW_ITEM_ID, CANNOT_SELL_ARROW_ID, LARGE_AMMOUNT_ARROW_ID };
...
typedef struct _Object
{
	...
	WORD itemrefine;			// 装备精炼
    WORD itemksAmulet;			// 护身符神兽宝石镶嵌次数
	...
	DWORD m4axhp;			   //sky 龙槽升级 最大生命值
	DWORD p4damage;			   //sky 龙槽升级 最小攻击力
	DWORD m4axpdamage;		   //sky 龙槽升级 最大攻击力
	DWORD m4damage;			   //sky 龙槽升级 最小法术攻击力
	DWORD m4axmdamage;		   //sky 龙槽升级 最大法术攻击力
	DWORD p4defence;		   //sky 龙槽升级 物防
	DWORD m4defence;		   //sky 龙槽升级 魔防

	DWORD m5axhp;			   //sky 龙星 最大生命值
	DWORD p5damage;			   //sky 龙星 最小攻击力
	DWORD m5axpdamage;		   //sky 龙星 最大攻击力
	DWORD m5damage;			   //sky 龙星 最小法术攻击力
	DWORD m5axmdamage;		   //sky 龙星 最大法术攻击力
	DWORD p5defence;		   //sky 龙星 物防
	DWORD m5defence;		   //sky 龙星 魔防
	...
	DWORD m9axhp;			   //sky 备用 最大生命值
	DWORD p9damage;			   //sky 备用 最小攻击力
	DWORD m9axpdamage;		   //sky 备用 最大攻击力
	DWORD m9damage;			   //sky 备用 最小法术攻击力
	DWORD m9axmdamage;		   //sky 备用 最大法术攻击力
	DWORD p9defence;		   //sky 备用 物防
	DWORD m9defence;		   //sky 备用 魔防

	DWORD m10axhp;			   //MYY 装备精炼 最大生命值
	DWORD p10damage;		   //MYY 装备精炼 最小攻击力
	DWORD m10axpdamage;		   //MYY 装备精炼 最大攻击力
	DWORD m10damage;		   //MYY 装备精炼 最小法术攻击力
	DWORD m10axmdamage;		   //MYY 装备精炼 最大法术攻击力
	DWORD p10defence;		   //MYY 装备精炼 物防
	DWORD m10defence;		   //MYY 装备精炼 魔防
	...
	DWORD m20axhp;			   //MYY 法宝功能 最大生命值
	DWORD p20damage;		   //MYY 法宝功能 最小攻击力
	DWORD m20axpdamage;		   //MYY 法宝功能 最大攻击力
	DWORD m20damage;		   //MYY 法宝功能 最小法术攻击力
	DWORD m20axmdamage;		   //MYY 法宝功能 最大法术攻击力
	DWORD p20defence;		   //MYY 法宝功能 物防
	DWORD m20defence;		   //MYY 法宝功能 魔防
	...
	DWORD m22axhp;			   //MYY 元神 最大生命值
	DWORD p22damage;		   //MYY 元神 最小攻击力
	DWORD m22axpdamage;		   //MYY 元神 最大攻击力
	DWORD m22damage;		   //MYY 元神 最小法术攻击力
	DWORD m22axmdamage;		   //MYY 元神 最大法术攻击力
	DWORD p22defence;		   //MYY 元神 物防
	DWORD m22defence;		   //MYY 元神 魔防
	...
	BYTE rdam; //伤害反射%x
	BYTE ignoredef; //%x
	BYTE addpdef; //soke 物理防御力增强
	BYTE addmdef; //soke 魔法防御力增强
	BYTE addpdam; //soke 增加物理攻击力
	BYTE addmdam; //soke 增加魔法攻击力
	BYTE addhp;   //soke 增加生命值
...
//	DWORD maxpdam; //最大攻击力
//	DWORD rdam; //最小远程伤害
//	DWORD maxrdam; //最大远程伤害
//	DWORD tdam; //最小仙术伤害
//	DWORD maxtdam; //最大仙术伤害
//	DWORD mdam; //最小法术攻击力
//	DWORD maxmdam; //最大法术攻击力
//	DWORD pdef; //最小防御力
//	DWORD maxpdef; //最大防御力
//	DWORD mdef; //最小法术防御力
//	DWORD maxmdef; //最大法术防御力
//	WORD hpr; //生命值恢复
//	WORD mpr; //法术值恢复
//	WORD spr; //体力值恢复
//	WORD atspeed; //攻击速度
...
//	WORD maxdur; //最大耐久度
//	DWORD price; //价格
//	WORD kind;	//物品类型,0普通,1蓝色,2金色,4神圣, 8 套装
//	BYTE upgrade;//物品升级等级
//	union{
//		struct{
//			WORD gold; //金
...
//	WORD chaosdef; //抗混乱增加
//	DWORD socket[6]; //孔
//	oskill skillo[6];//增加技能
//	WORD reflectp; //伤害反射x％
//	WORD reflect; //伤害反射（人物等级/x）
//	BYTE durpoint; //恢复装备耐久度点数
//	BYTE dursecond; //恢复装备耐久度描述
//	WORD holy; //神圣一击
//	WORD ehpp; //增加伤害值x％
//	WORD ehpv; //增加伤害值（人物等级/x）
//	WORD epdamp; //增加攻击力x％
//	WORD epdamv; //增加攻击力（人物等级/x）
//	WORD erdamp; //增加远程伤害x%
//	WORD erdamv; //增加远程伤害（人物等级/x）
//	WORD etdamp; //增加仙术x%
//	WORD etdamv; //增加仙术(人物等级/x)
//	WORD emdamp; //增加法术攻击力x%
//	WORD emdamv; //增加法术攻击力(人物等级/x)
//	WORD fivepro; //五行属性增加
//	WORD hpleech; //x%吸收生命值y％
//	WORD hpleech_value; 
//	WORD mpleech; //x%吸收法术值y％
//	WORD mpleech_value;
//	WORD hptomp; //转换生命值为法术值x％
//	WORD dhpv; //物理伤害值减少(人物等级/x)
//	WORD dhpp; //物理伤害减少x%
//	WORD dmpv; //法术伤害值减少(人物等级/x)
//	WORD dmpp; //法术伤害值减少x%
//	WORD invtarget; //x%忽视目标防御
//	WORD incgold; //增加银子掉落x%
//	WORD cold; //x%使目标冰冻
//	WORD poison; //x%使目标中毒
//	WORD petrify; //x%使目标石化
//	WORD chaos; //x%使目标混乱
//	WORD blind; //x%使目标失明
//	WORD mf; //增加掉宝率x%
//	WORD charm; //魅力增加x
//	WORD sleight; //熟练度增加x%
//	DWORD exp; //物品经验
//	char maker[MAX_NAMESIZE]; //打造者名称
//	char set[MAX_NAMESIZE]; //套装名称
//}t_Object;
