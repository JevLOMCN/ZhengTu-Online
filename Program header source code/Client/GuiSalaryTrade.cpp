 ...
 * \return 无
 */
CGuiSalaryTrade::CGuiSalaryTrade(void)
: m_dwRepair(CGuiNpcDialog::eSalary)
{
	FUNCTION_BEGIN;
 ...
 * \return 无
 */
void CGuiSalaryTrade::OnClose(void)
{
	FUNCTION_BEGIN;
	...
	GetGameGuiManager()->m_guiSalaryTrade = NULL;
...
void CGuiSalaryTrade::OnCreate()
{
	...
	m2_pStaMoney = GetEditBox(1002); //soke 工资显示
	m2_pStaMoney->SetEnabled(false);
...
//soke 增加窗口支持工资显示
HRESULT CGuiSalaryTrade::OnRender(float fElapsedTime)
{
	FUNCTION_BEGIN;
	...
	// 刷新当前工资数
	DWORD dwMoney = GetScene()->GetMainCharacter()->GetProperty()->salary;
	m2_pStaMoney->SetText(Money2String(dwMoney));
 ...
 * \return 无
 */
void CGuiSalaryTrade::AddBuyObject(ObjectBase_t* pObject,int nNum,int nLevel)
{
	FUNCTION_BEGIN;
 ...
 * \return 无
 */
void CGuiSalaryTrade::AddBuyPage(const char* pageName,const char* pszBuyParam)
{
	FUNCTION_BEGIN;
...
void CGuiSalaryTrade::SetPage(int nPage)
{
...
* \return 无
*/
void CGuiSalaryTrade::SetParam(std::vector<std::string> & params)
{
	FUNCTION_BEGIN;
	...
	if(m_dwRepair!=CGuiNpcDialog::eSalary)
	{
		if(params.size() > index)
		{
...
* \return 检查结果
*/
bool CGuiSalaryTrade::IsBuyObject(DWORD dwObjectID)
{
	FUNCTION_BEGIN;
...
* \return 检查结果
*/
bool CGuiSalaryTrade::IsSellObject(DWORD dwObjectID)
{
	FUNCTION_BEGIN;
...
* \return 检查结果
*/
bool CGuiSalaryTrade::IsRepairObject(DWORD dwObjectID)
{
	FUNCTION_BEGIN;
...
* \return 无
*/
void CGuiSalaryTrade::RefreshRepairMoney()
{
	FUNCTION_BEGIN;
...
* \return 消息处理结果
*/
bool CGuiSalaryTrade::OnGuiEvent(UINT nEvent,UINT nID,CGuiControl* pControl)
{
	FUNCTION_BEGIN;
