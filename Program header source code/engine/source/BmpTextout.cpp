...
#include "..\include\bmptextout.h"
#include "../../xml_parse/XmlParseEx.h"

/**
* \brief 得到当前CBmpTextout对象指针
* 
* \return 当前CBmpTextout静态对象指针
*/
CBmpTextout* GetBmpTextout()
{
	static CBmpTextout  sm;
	return &sm;
}
 ...
 * \brief 构造函数
 * 
 * 装载索引表(data\datas.pak-->datas\\BmpText.xml)
 *
 */
CBmpTextout::CBmpTextout(void)
{
	SetResourceLocation("data\\Interfaces.gl",17);

	TiXmlDocument doc;
	Stream* pStream = OpenPackFileForRead("data\\datas.pak","datas\\BmpText.xml");
	if(!pStream)
	{
		Engine_WriteLog("装载 BmpText.xml 失败\n");
		return ;
	}
	if(!LoadXMLFromStream(pStream,doc))
	{
		ClosePackFileForRead(pStream);
		Engine_WriteLog("解析 BmpText.xml 失败\n");
		return;
	}
	wchar_t wString[256];
...
* \return void
*/
void CBmpTextout::SetResourceLocation(const char* fileName,int group)
{
...
* \return void
*/
void CBmpTextout::SetCharPicture(WORD ch,int frame)
{
 ...
 * \return void
 */
void CBmpTextout::DrawString(POINT pt,const char* pszString,int count,DWORD color,stPointF * scale)
{
...
* \return 字符串的绘制区域大小
*/
SIZE CBmpTextout::GetStringSize(const char* pszString,int count)
{
	SIZE RectSize = SIZE();
	RectSize.cy = 0;
