...
{
	//Zebra::logger->debug("zSkill::create(%s,id=%u level=%u)", pEntry->name, id, level);
	...
	{
		Zebra::logger->debug("zSkill::create():无法分配有效的tempid");
		SAFE_DELETE(ret);
		return NULL;
	}
	// */
	...
	if (pEntry->getType() == zSceneEntry::SceneEntry_Player /*&& pEntry->getPriv()<=Gm::normal_mode*/)
	{
		...
		if (pEntry->usm.getPointInTree(ret->base->kind, ret->base->subkind) < ret->base->needpoint)
		{
			SAFE_DELETE(ret);
			return NULL;
		}
		...
		if (this->istemp||_entry->needWeapon(curRevCmd->wdMagicType)) // 检查施放技能是否需要武器
		{
			...
			if(_entry->checkSkillCost(actionbase))	// 计算自身各种消耗
			{
				if (_entry->checkPercent())	// 判断技能施放成功的几率
				{
					std::vector<SkillStatus>::const_iterator iter;
					//soke 修复弓手战旗可以用技能
					if (_entry->checkReduce(actionbase->objcost,actionbase->objnum) || actionbase->objcost == 21 || actionbase->objcost == 142 )
					{
						showMagicToAll();
							...
							case 319: // 尸暴术
								{
									...
									if (pNpc)
									{
										if (pNpc->getState() == zSceneEntry::SceneEntry_Death && !pNpc->isUse) 
										{
											...
											pNpc->scene->sendCmdToNine(pNpc->getPosI(), &removeNpc, sizeof(removeNpc));
										}
										else
										{
											return false;
...
//-技能范围测试
							...
							if (base)
							{
								zObject *o=zObject::create(base,zMisc::randBetween(1, 10));
								_entry->scene->addObject(o,pd);
							}
#endif
							DWORD num=0;
