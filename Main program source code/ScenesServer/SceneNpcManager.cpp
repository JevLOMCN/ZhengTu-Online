...
#ifdef _ZJW_DEBUG	
	Zebra::logger->debug("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SceneNpcManger构造");
#endif		
}
...
{
#ifdef _ZJW_DEBUG	
	Zebra::logger->debug("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SceneNpcManger析构");
#endif		
...
void SceneNpcManager::removeNpcInOneScene(Scene *scene)
{
	...
	for(std::vector<DWORD>::iterator iter = exec.del_vec.begin() ; iter != exec.del_vec.end() ; iter ++)
	{
		...
		if(pNpc)
		{
			...
			SceneNpcManager::getMe().removeSpecialNpc(pNpc); //醉梦修复动态地图
			SAFE_DELETE(pNpc);
		}
	}
}
...
bool SceneNpcManager::loadNpcCommonChatTable()
{
	...
	{
		Zebra::logger->error("不能读取npc聊天内容文件 %s", (Zebra::global["mapdir"] + "NpcCommonChat.xml").c_str());
		return false;
	}
