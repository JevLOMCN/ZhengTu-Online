...
bool SceneUser::npcTradeChongzhi(Cmd::stBuyObjectChongzhiTradeUserCmd *ptCmd,zObjectB *base, BYTE itemlevel)
{
	SceneNpc * n = SceneNpcManager::getMe().getNpcByTempID(npc_dwNpcTempID);
	if (!n)
	{
		Zebra::logger->debug("[交易:玩家<------充值点商店]%s 交易时，找不到该npc tempID=%u", name, npc_dwNpcTempID);
		return false;
	}
	DWORD need_chongzhi=0;
	
	//zise 物品等级价格 比如材料等级
	float levelopt = pow(5,itemlevel);
	//zise 2017-2-4 充值点购买
	if(base && charbase.chongzhi >= (base->price)*ptCmd->dwNum*levelopt)
	//if(base && charbase.chongzhi >= base->price*ptCmd->dwNum*levelopt)
	{
					...
					if (packs.addObject(o, true, AUTO_PACK)) 
{
				...
				if (callback.num() || free) 
				{
					...
					//zise 修改充值点支持购买等级
					//DWORD need_chongzhi = base->price*count;
					
					//zise 2017-2-4 修改充值点购买
					DWORD need = (base->price)*count*(DWORD)((float)levelopt);
				//	DWORD need = base->price*count*(DWORD)((float)levelopt);
					if(charbase.chongzhi >=need)
					{
						need_chongzhi=need;
					}
					else
					{
						need_chongzhi=charbase.chongzhi;
					}
					if(need_chongzhi)
					{
						std::string disc = "买东西:"; 
						disc += o->base->name;
						if (!packs.removeChongzhi(need_chongzhi,disc.c_str())) 
						{
							Zebra::logger->fatal("[交易:玩家<------充值点商店]用户(%s)买%s时充值点计算错误!", name, o->base->name);
						}
					}
					...
					if(need_chongzhi)
					{
                        Zebra::logger->trace("[交易:玩家<------充值点商店]用户(%s,%u)买%s(%d)个花费充值点%d点",name,id,o->name,count,need_chongzhi);
						Channel::sendChongzhi(this, Cmd::INFO_TYPE_GAME , need_chongzhi,"得到物品 %s(%d)个 , 花费充值点",o->name , count);
					}
				}
				if (!free) 
				{ //package is full
